...
const renameImageFile = (newName, baseUrl) => {
    ...
    rl.question('Enter the folder path: ', (folderPath) => {
        ...
        fs.readdir(folderPath, (err, files) => {
            ...
            const renamePromises = jpgFiles.map((file, index) => {
                return new Promise((resolve, reject) => {
                    ...
                    if (productName) {
                        ...
                        fs.rename(originalFilePath, newFilePath, (err) => {
                            if (err) {
                                console.error(`Error renaming file ${originalFilePath} to ${newFilePath}:`, err);
                                reject(err);
                            } else {
                                console.log(`File renamed successfully: ${originalFilePath} -> ${newFilePath}`);
